...
For multi-tool servers, see the three taxonomy patterns (flat, per-tool tiered, per-result) in a dedicated Apify monetization guide (§ "Pricing structure for MCP").

**Price point for first-time publishers:** $0.001–$0.01 per tool call is the empirical range for read-only lookups; $0.02–$0.10 for AI-LLM-backed tools. Start at the low end during the 30-day calibration window, then raise (a 14-day delay applies to price increases on the Store). **Do not guess a price** — consult a dedicated Apify pricing-strategy guide before publishing to the Store.
...
All charging in the Actor goes through this helper. New code can't bypass the rules accidentally. For the full PPE doctrine, consult a dedicated Apify monetization guide.
...
The `fiery_dream/mcp-server-generator` Actor produces working MCP servers but does not wire `Actor.charge()`. You must add it after generation. The minimal edit:
...
A general Apify/scraping patterns guide (§ "KV Store cache pattern") has the full helper with idiomatic naming conventions.
...
For tools that scrape (not REST), use `Actor.createProxyConfiguration({ groups: ['RESIDENTIAL'] })` to spread requests across many IPs. Overkill for documented REST APIs; consult a scraping guide for when this fits.
...
- **PPE event taxonomy design** (which tools to map to which events, free-tier handling, idle-cost trade-offs) → a dedicated Apify monetization guide
- **README, Store description, input/dataset schemas for MCP variants** → a dedicated Apify Actor content guide
- **Crawlee/Playwright inside MCP tools (for tools that scrape)** → a scraping guide + this skill's cold-start guidance
