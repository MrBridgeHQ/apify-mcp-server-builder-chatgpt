...
description: Build, modify, debug, or review MCP server Actors on Apify, including Standby mode, Streamable HTTP, webServerMcpPath, ACTOR_WEB_SERVER_PORT, TypeScript or Python MCP SDKs, proxy and from-scratch templates, Apify MCP URLs, local testing, and deployment.
---
...
- **PPE / pricing / event taxonomy for MCP** → a dedicated Apify monetization guide
- **README + input/dataset schemas + Store description for MCP variants** → a dedicated Apify Actor content guide
- **General Apify Actor patterns (actor.json, KV cache, status messages, base Docker images)** → a general Apify/scraping patterns guide
- **Graceful-exit error handling that applies inside MCP tool handlers** → a graceful-exit / error-handling guide
